...
export default function AdminQuestions() {
    ...
    const handleSubmit = async () => {
        ...
        await axios.post('/api/admin/questions', { text: question, image: questionImage, options });
        setQuestion('');
    ...
    return (
        <div className='px-40 py-4 h-screen overflow-auto'>
            ...
            <div className="mb-6 space-y-2">
                ...
                />
                <label className="block mb-2 font-medium">Question Image (optional)</label>
                <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => {
                ...
                />
                {questionImage && (
                    <img src={questionImage} alt="question" className="w-32 h-32 object-cover mt-2" />
                )}
                ...
                {options.map((opt, i) => (
                    <div key={i} className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        ...
                        {opt.image && (
                            <img src={opt.image} alt={`option-${i}`} className="w-20 h-20 object-cover" />
                        )}
                ...
                {filteredQuestions.map((q) => (
                    <li key={q._id} className="p-4 rounded shadow shadow-amber-50">
                        <div className="flex justify-between items-center">
                            ...
                            {q.image && <img src={q.image} alt="question" className="w-32 h-32 object-cover my-2" />}
                        ...
                        <ul className="mt-2 list-disc pl-5">
                            {q.options.map((opt, i) => (
                                <li key={i} className={opt.isCorrect ? 'text-green-600 font-semibold' : ''}>
                                    ...
                                    {opt.image && (
                                        <div>
                                            <img src={opt.image} alt="option" className="w-20 h-20 object-cover mt-1" />
                                        </div>
                                    )}
                                </li>
                            ))}
